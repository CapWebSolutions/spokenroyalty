<?php
/*
* Child Theme Name: Spoken Royalty
* Author: Cap Web Solutions
* Url: https://capwebsolutions.com/
*/

/* --- DEFINE CHILD THEME --- */

// CHILD_DIR defined in Genesis
define( 'CHILD_THEME_NAME', 'spoken-royalty' );
define( 'CHILD_THEME_URL', 'http://www.cws-genesis-child.com/' );
...
add_action('genesis_setup','cws_theme_setup', 15);
function cws_theme_setup() {
	...
	add_filter( 'http_request_args', 'cws_dont_update', 5, 2 );
	...
	add_action( 'widgets_init', 'cws_remove_genesis_widgets', 20 );
	function cws_remove_genesis_widgets() {
	...
	// See /lib/child_setup.php
	add_action( 'wp_enqueue_scripts', 'cws_scripts_and_styles', 999 );

	// Add viewport meta tag for mobile browsers @since GENESIS 2.0
	add_theme_support( 'genesis-responsive-viewport' );

	// Change favicon location
	add_filter( 'genesis_pre_load_favicon', 'cws_custom_favicon_location' );
	...
	/*
		...
		* For example, to call the 225 x 225 sized image, we would use the function:
		  <?php the_post_thumbnail( 'cws_medium_img' ); ?>

		* You can change the names and dimensions to whatever you like.
	*/

	add_image_size( 'cws_featured_img', 600, 400, TRUE );
	...
	add_filter( 'posts_where', 'cws_hide_media_library' );
	function cws_hide_media_library( $where ){
	...
	add_filter( 'genesis_footer_creds_text', 'cws_footer_cred' );
